...
class RealDataHyperedgeDynamicsM2:
    ...
    def identify_triangles(self):
        ...
        print("Identifying triangle interactions for m=2...")
        ...
        print(f"Identified {total_hyperedges} triangle interactions (m=2)")

    def calculate_activity_vulnerability_m2(self):
        """Calculate node activity and vulnerability for m=2 hyperedges"""
        print("Calculating activity and vulnerability for m=2...")

        # Track node presence in triangles
        node_triangle_windows = defaultdict(set)
        node_active_windows = defaultdict(list)

        # First pass: find all time windows where each node appears in a triangle
        for time_win, triangles in self.hyper_edges.items():
            for triangle in triangles:
                for node in triangle:
                    node_triangle_windows[node].add(time_win)
                    node_active_windows[node].append(time_win)

        # Calculate activity: proportion of time windows with triangle participation
        total_steps = len(self.time_windows)
        for node in self.nodes:
            active_count = len(node_triangle_windows.get(node, set()))
        ...
        for node in tqdm(self.nodes, desc="Calculating vulnerability"):
            ...
            for i in range(len(active_windows)):
                ...
                if current_window <= max(self.time_windows) - inactive_threshold:
                    # Check if there's another triangle within the threshold
                    found_next = False
                    ...
                    # If no triangle found within threshold, count as interruption
                    if not found_next:
        ...
        print("Activity and vulnerability calculation for m=2 completed")

    def find_most_frequent_hyperedge(self):
        """Find the most frequent hyperedge (triangle) in the dataset"""
        ...
        if hyperedge_counts:
            ...
            # Fallback: use first three nodes
            nodes_list = list(self.nodes)[:3]
    ...
    def simulate_hyperedge_dynamics(self, n, m, T, hyperedge_nodes):
        ...
        :param m: hyperedge order (m=2 for triangles)
        :param T: total time
    ...
    def analyze_hyperedge_dynamics_m2(self, num_simulations=500):
        """
        Analyze hyperedge dynamics for m=2 using real data
        """
        n = len(self.nodes)
        m = 2  # Fixed for triangle hyperedges
        ...
        # Visualization
        self._plot_results_m2(T_range, theory_P_smallT, theory_P_largeT,
    ...
    def _plot_results_m2(self, T_range, theory_P_smallT, theory_P_largeT,
                         theory_P_exact, sim_P, n, m, hyperedge_nodes):
        """Plot results for m=2 similar to Fig.1 in the paper"""
...
def main():
    """Main function to run the analysis for m=2"""
    # Set file path
    ...
    print("=== Real Data Hyperedge Dynamics Analyzer (m=2) ===")
    print(f"Loading dataset: {dataset_path}")

    # Create analyzer instance
    analyzer = RealDataHyperedgeDynamicsM2(dataset_path)

    # Step 1: Load and prepare data
    analyzer.load_and_prepare_data()

    # Step 2: Identify triangles (m=2 hyperedges)
    analyzer.identify_triangles()

    # Step 3: Calculate activity and vulnerability for m=2
    analyzer.calculate_activity_vulnerability_m2()

    # Step 4: Analyze hyperedge dynamics
    results = analyzer.analyze_hyperedge_dynamics_m2(num_simulations=500)

    # Print summary
    print("\n" + "=" * 50)
    print("Analysis Summary for m=2")
    print("=" * 50)
    print(f"Network size: n = {len(analyzer.nodes)}")
    print(f"Hyperedge order: m = 2")
    ...
    n_power_m = len(analyzer.nodes) ** 2
    ...
    print("=" * 50)
    print("The selected hyperedge (triangle) is chosen based on:")
    print("1. Maximum frequency: The triangle that appears most often in the dataset")
    ...
    print("4. For m=2, we identify triangles (3-cliques) in the temporal network")
